...
export default function SearchOutput({ seacrhText, searchData }) {
    return (
            ...
            <div>
                {
                    searchData && searchData.length <= 0 ?
                        ...
                        :
                        searchData.map(item => (
                            <Link key={item._id} href={item._id} className='w-full inline-block my-3 border-b hover:text-blue-800 transition-all'>
                                {item.name}
                            </Link>
                        ))
                }
